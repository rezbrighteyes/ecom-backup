...
def _scope_views_to_website(env):
    website = env['website'].search([('name', '=', SS_WEBSITE_NAME)], limit=1)
    if not website:
        _logger.warning(
            'sunglass_superstore_product_page: website "%s" not found, skipping view scoping',
            SS_WEBSITE_NAME
        )
        return
    ...
    for view in views:
        if view.website_id.id != website.id:
            ...
            _logger.info('Scoped view %s to website_id=%s', view.key, website.id)
