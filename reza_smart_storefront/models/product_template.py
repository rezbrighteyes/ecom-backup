...
class ProductTemplateSF(models.Model):
    ...
    def _sf_is_available(self):
        ...
        if not variant_ids:
            return False
        # Check stock across ALL companies
        self.env.cr.execute("""
    ...
    def _sf_get_cross_sells(self, limit=4):
        ...
        if website_id:
            base_domain.append(('website_id', 'in', [website_id, False]))

        # Try 1: same public category
        if self.public_categ_ids:
            domain = base_domain + [('public_categ_ids', 'in', self.public_categ_ids.ids)]
        ...
        # Try 2: same internal category
        if self.categ_id:
            domain = base_domain + [('categ_id', '=', self.categ_id.id)]
            products = self.env['product.template'].sudo().search(domain, limit=limit * 5)
            in_stock = products.filtered(lambda p: p._sf_is_available())
            if in_stock:
                return in_stock[:limit]

        # Try 3: any available product on this website
        products = self.env['product.template'].sudo().search(base_domain, limit=limit * 5)
        in_stock = products.filtered(lambda p: p._sf_is_available())
        return in_stock[:limit]
