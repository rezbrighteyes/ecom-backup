...
class StockMoveLine(models.Model):
    ...
    def manage_package_type(self):
        ...
        return res
    related_package_ids = fields.Many2many(
    'stock.package',
    string="Related Packages",
    compute='_compute_related_package_ids',
    store=False 
    )
    @api.depends('picking_id','product_id')
    def _compute_related_package_ids(self):
        related = False
        if self.picking_id.sale_id.create_package=='auto':
            related = self.picking_id.sale_id.wk_packaging_ids.mapped('pckage_id')
        elif self.picking_id.sale_id.create_package=='manual':
            related = self.picking_id.sale_id.wk_manual_packaging_ids.mapped('package_id')
        if related:
            self.related_package_ids = related
        else:
            self.related_package_ids = self.env['stock.package'].search([])
...
class ChooseDeliveryPackage(models.TransientModel):
    ...
    _check_positive_cover_amount = models.Constraint(
        "CHECK(cover_amount >= 0)",
        "Cover Amount must be positive (cover_amount>=0).",
    ),

    _check_positive_shipping_weight = models.Constraint(
    ...
    def onchange_delivery_packaging_id(self):
        packaging_id = self.package_type_id
        # _logger.info("--------------- called %r--------------" %self.package_type_id)
...
class StockQuantPackage(models.Model):
    ...
    @api.onchange('package_type_id')
    def chnage_package_type_id(self):
        self.height = self.package_type_id.height
        self.width = self.package_type_id.width
        self.length = self.package_type_id.packaging_length
        self.weight = self.package_type_id.base_weight
        self.cover_amount = self.package_type_id.cover_amount
