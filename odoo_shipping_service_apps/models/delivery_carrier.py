...
DeliveryType = [("fixed", "Fixed Price"), ("base_on_rule", "Based on Rules")]
APIUoM = [("LB", "LB"), ("KG", "KG"), ("OZ", "OZ")]
...
class DeliveryCarrier(models.Model):
    ...
    def wk_get_shipping_price_from_so(self, orders):
        for order in orders:
            if order.carrier_id.delivery_type not in ["fixed", "base_on_rule"]:
                if (
                    order.create_package == "manual"
                    and len(order.wk_packaging_ids) == 0
                ):
                    raise ValidationError(
                        _(
                            "Create the package first for manual packaging of order %s."
                            % (order.name)
                        )
                    )
    ...
    _extra_price_source_selection = (
        lambda self, *args, **kwargs: self._get_extra_price_source(*args, **kwargs)
    )

    image = fields.Binary(string="Website Image")
    multi_package = fields.Boolean(
        string="Multi Package",
        help="Enable it if shipping method support multi package '",
        compute="_compute_multi_package",store=True
    )
    ...
    extra_service_price = fields.Float(
        string="Extra Service Charges", help="Extra  Added Service Charge "
    )
    ...
    # packaging_id = fields.Many2one(comodel_name="product.packaging", string="Packaging") # place of this use packaging_type_id
    ...
    package_creation_type = fields.Selection(
        [
            ("per_qty", "A package for each quantity of product"),
            ("per_line", "A package per order line"),
            ("all_products", "A package for all products"),
        ],
    ...
    def get_package_attribute(self, line, packaging_id, partial_package):
        ...
        if not partial_package:
            ...
            product_qty, product_weight = (
                int(line.product_uom_qty) and int(line.product_uom_qty) or 1,
                product_id.weight and product_id.weight or 1,
            )
        else:
    ...
    def wk_get_product_package(self, line, packaging_id, partial_package=None):
        ...
        max_qty, max_weight = (
            int(packaging_id.wk_max_qty) and int(packaging_id.wk_max_qty) or 1,
            packaging_id.max_weight
            and packaging_id.max_weight
            or 1,
        )
        ...
        if qty_capacity and qty_capacity < max_qty:
            ...
            if package_count:
                ...
                multi_pckg_qty_capacity.update(
                    dict(
            ...
            if product_qty % qty_capacity:
                ...
                single_pckg_qty_capacity.update(
                    dict(
    ...
    def wk_validate_data(self, order=None, pickings=None):
        if pickings:
            if not pickings.move_line_ids.result_package_id:
                raise ValidationError("Create the package before sending to shipper.")
            else:
                package_ids = pickings.move_line_ids.result_package_id.filtered(
                    lambda package_id: not package_id.package_type_id
                )
                if len(package_ids):
                    raise ValidationError(
                        "Packaging is not set for package %s."
                        % (",".join(package_ids.mapped("name")))
                    )

    @api.model
    def wk_get_packaging_id(self, product_id=None, package_type_id=None):
        pck_type = self.packaging_type_id
        packaging_type = None
        if product_id:
            packaging_types = product_id.wk_packaging_ids.filtered(
                lambda pt: pt.package_carrier_type == self.delivery_type
            )
            packaging_type = packaging_types and packaging_types[0] or pck_type
        elif package_type_id:
            packaging_type = package_type_id or pck_type
        if packaging_type:
        ...
        if order:
            for line in order.order_line:
                ...
                if not product_id or line.is_delivery:
                    continue
                packaging_type = self.wk_get_packaging_id(product_id=product_id)
                packagings[packaging_type].append(line)
        else:
            for package in pickings.move_line_ids.result_package_id:
                packaging_type = self.wk_get_packaging_id(package_type_id=package.package_type_id)
                packagings[packaging_type].append(package)
    ...
    def wk_merge_half_package(self, items):
        ...
        for item in filter(lambda item: not item.get("full_capacity"), items):
            packaging_type_id = item.get("packaging_id")

            # packaging_id now directly refers to stock.package.type
            if isinstance(packaging_type_id, int):
                packaging_type = self.env["stock.package.type"].browse(packaging_type_id)
            else:
                packaging_type = packaging_type_id  # already a record

            data[packaging_type].append(item)
        ...
        for packaging_type, lines in new_dict.items():
            items.extend(
                self.wk_get_product_package(lines, packaging_type, partial_package=True)
            )

        return items


    def auto_package(self, order):
        if order.carrier_id.package_creation_type != "all_products":
            order.generate_sale_packages()
            return [
                {
                    "packaging_id": (
                        pkg.package_id.package_type_id.id
                        if pkg.package_id.id
                        else pkg.packaging_id.id
                    ),
                    "weight": pkg.weight,
                    "width": pkg.width,
                    "length": pkg.length,
                    "height": pkg.height,
                    "cover_amount": pkg.cover_amount
                }
                for pkg in order.wk_packaging_ids
            ]
        else:
            package = (
                order.package_id
                if order.package_id
                else order.carrier_id.packaging_type_id
            )
            weight = 0
            for line in order.order_line:
            ...
            width = package.width
            length = package.length if order.package_id else package.packaging_length
            weight += (
                package.shipping_weight if order.package_id else package.base_weight
            )
            return [
                {
                    "packaging_id": (
                        order.package_id.package_type_id.id
                        if order.package_id
                        else order.carrier_id.packaging_type_id.id
                    ),
                    "weight": weight,
                    "width": width,
                    "length": length,
                    "height": height,
                    'cover_amount': order.package_id.cover_amount
                }
            ]
    ...
    def wk_get_order_package(self, order):
        ...
        if order.create_package == "auto":
            if order.carrier_id.multi_package:
                result = self.auto_package(order)
            else:
        ...
        elif order.create_package == "manual":
            if not order.wk_manual_packaging_ids:
                raise ValidationError(
                    "Kindly create manual packages first than click on get rate!"
                )
            return [
                {
                    "packaging_id": (
                        pkg.product_package_id.package_type_id.id
                        if pkg.product_package_id.id
                        else pkg.carrier_package_id.id
                    ),
                    "weight": pkg.weight,
                    "width": pkg.width,
                    "length": pkg.length,
                ...
                for pkg in order.wk_manual_packaging_ids
            ]
    ...
    def get_package_count(self, weight_limit, order=None, pickings=None):
        WeightValue = (
            self._get_weight(order=order)
            if order
            else self._get_weight(pickings=pickings)
        )
        ...
        return (
            ...
            weight_limit,
            last_package,
            total_package + int(bool(last_package)),
        )

    @api.model
    def _get_api_weight(self, shipping_weight):
        q = self._get_default_uom()._compute_quantity(1, self.uom_id)
    ...
    def _get_per_order_line_weight(self, line):
        return (
            line.product_id.weight or 0.0
        ) * self._get_default_uom()._compute_quantity(line.product_uom_qty, self.uom_id)
            ...
            if pickings.sale_id and pickings.sale_id.currency_id:
                ...
                if not currency:
                    warehouse = (
                        ...
                        and pickings.picking_type_id.warehouse_id
                    )
                    if warehouse:
                        currency = (
                            warehouse.property_product_pricelist
    ...
    def _get_shipment_address(self, data, entity):
        if (
            data
            and not data.get("company_name")
            and hasattr(entity, "company_name")
            and entity.company_name
        ):
