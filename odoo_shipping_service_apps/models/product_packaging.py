...
AmountOption = [("fixed", "Fixed Amount"), ("percentage", "%  of Product Price")]
...
class ProductPackaging(models.Model):
    ...
    wk_max_qty = fields.Float('Maximum Quantity', default=1, digits='Product Unit of Measure', help="Maximum Quantity of products contained in the packaging.")

    @api.constrains("wk_max_qty")
    def _check_wk_max_qty(self):
        for rec in self:
            if rec.wk_max_qty < 1:
                raise ValidationError(
                    "Maximum Quantity must always be greater than or equal to 1."
                )